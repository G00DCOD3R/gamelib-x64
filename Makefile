...
out/boot.o: src/bootloader/*.s | out
	$(AS) src/bootloader/*.s -o $@ -gstabs+

out/kernel.o: src/kernel/*.s src/game/*.s | out
	$(AS) src/kernel/*.s src/game/*.s -o $@ -gstabs+
...
gdb: all
	$(LD) -nostdlib -g --oformat=symbolsrec -T src/bootloader/link_boot.ld -o out/boot.srec out/boot.o
	$(LD) -nostdlib -g --oformat=symbolsrec -T src/kernel/link_kernel.ld -o out/kernel.srec out/kernel.o
	qemu-system-x86_64 -S -s -enable-kvm -cpu host HD_img &
	sleep 1
